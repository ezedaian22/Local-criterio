  ...
  return (
    <div className="min-h-screen bg-criterio-negro">
      ...
      <header className="border-b border-criterio-gris3 bg-criterio-gris/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div>
              <h1 className="font-display text-xl font-semibold text-criterio-blanco leading-none">
                Criterio
              </h1>
              <p className="text-criterio-acento font-mono text-[10px] tracking-[0.2em] uppercase">
                {perfil?.rol === 'gerencia' ? 'Gerencia' : perfil?.locales?.nombre}
              </p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-criterio-texto/40 font-mono text-xs hidden sm:block">
              {perfil?.nombre}
            </span>
            <button
              onClick={logout}
              className="text-xs font-mono text-criterio-texto/40 hover:text-criterio-acento
                         border border-criterio-gris3 px-3 py-1.5 rounded-lg hover:border-criterio-acento
                         transition-colors"
            >
              salir
            </button>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        {titulo && (
          <div className="mb-6">
            <h2 className="font-display text-2xl font-semibold text-criterio-blanco">{titulo}</h2>
            <div className="w-8 h-0.5 bg-criterio-acento mt-2" />
          </div>
        )}
        {children}
      </main>
