          ...
          <div className="flex flex-col gap-4">
            <div className="card">
              ...
              <div className="flex gap-2 mb-4 flex-wrap">
                {[['mes','Mes vs Mes'],['anio','Año vs Año']].map(([k,l])=>(
              ...
              <div className="card">
                ...
                <div className="grid grid-cols-3 gap-4 font-mono text-sm">
                  {[['Ventas','ventas','text-criterio-acento'],['Gastos','gastos','text-red-400'],['Cant. ventas','cant','text-criterio-texto']].map(([l,k,c])=>(
                    <div key={k}>
                      <p className="text-criterio-texto/50 text-xs uppercase tracking-widest mb-2">{l}</p>
                      <p className={`text-lg ${c}`}>{k==='cant'?cmpData1[0]?.[k]:formatPesoFull(cmpData1[0]?.[k])}</p>
                      <p className="text-criterio-texto/40 text-xs mt-1">{MESES[cmpMes1]}</p>
                      <p className={`text-lg ${c} mt-2`}>{k==='cant'?cmpData2[0]?.[k]:formatPesoFull(cmpData2[0]?.[k])}</p>
                      <p className="text-criterio-texto/40 text-xs mt-1">{MESES[cmpMes2]}</p>
                      {cmpData1[0]?.[k]>0&&(
                        <p className={`text-xs mt-2 font-bold ${cmpData2[0]?.[k]>=cmpData1[0]?.[k]?'text-green-400':'text-red-400'}`}>
                          {cmpData2[0]?.[k]>=cmpData1[0]?.[k]?'▲':'▼'} {Math.abs(((cmpData2[0]?.[k]-cmpData1[0]?.[k])/cmpData1[0]?.[k])*100).toFixed(1)}%
                        </p>
                      )}
                    </div>
                  ))}
